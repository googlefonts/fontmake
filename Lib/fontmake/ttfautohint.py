...
def ttfautohint(in_file, out_file, args=None, **kwargs):
    ...
    boolean_options = (
        ...
        'detailed_info', 'no_info', 'adjust_subglyphs', 'symbol', 'ttfa_table',
        'verbose', 'version', 'windows_compatibility')
    other_options = (
        'default_script', 'fallback_script', 'family_suffix', 'hinting_limit',
        'fallback_stem_width', 'hinting_range_min', 'control_file',
        'hinting_range_max', 'strong_stem_width', 'increase_x_height',
    ...
    for option in other_options:
        ...
        if arg is not None:
            arg_list.append('--%s=%s' % (option.replace('_', '-'), arg))
